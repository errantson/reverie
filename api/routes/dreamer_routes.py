...
def calculate_spectrum():
    ...
    try:
        ...
        try:
            ...
            # If image exists, include URL in response; otherwise generate async
            if os.path.exists(image_path):
                ...
                print(f"🎨 Triggering async origincard generation for {safe_handle}")
                import threading
                _sp = {k: (spectrum_values.get(k) or 0) for k in
                       ('oblivion', 'authority', 'skeptic', 'receptive', 'liberty', 'entropy')}
                _sp.update({
                    'origin_oblivion': _sp['oblivion'],
                    'origin_authority': _sp['authority'],
                    'origin_skeptic': _sp['skeptic'],
                    'origin_receptive': _sp['receptive'],
                    'origin_liberty': _sp['liberty'],
                    'origin_entropy': _sp['entropy'],
                    'octant': octant,
                    'origin_octant': octant,
                })
                _handle_snap = safe_handle
                _dn_snap = display_name or handle
                _av_snap = avatar
                ...
                def _gen_card():
                    try:
                        ...
                        if gen_resp.status_code == 200:
                            print(f"[CALCULATE] Generated card for {_handle_snap}")
                        else:
                            print(f"[CALCULATE] Card generation failed ({gen_resp.status_code}) for {_handle_snap}")
                    except Exception as _e:
                        print(f"[CALCULATE] Card generation error for {_handle_snap}: {_e}")

                threading.Thread(target=_gen_card, daemon=True).start()

class Dashboard {
            ...
            <div class="compose-container">
                ...
                <div class="compose-footer">
                    <div class="compose-footer-left">
                        <button class="compose-schedule-icon-btn" id="composeScheduleIconBtn" onclick="window.dashboardWidget.openCalendarPicker()" title="Schedule for later">
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    async submitCompose() {
        ...
        try {
            ...
            if (isScheduling) {
                // SCHEDULING PATH - requires app password (handled server-side)
                // Note: Scheduling still uses backend because it needs to store for later execution
    ...
    async openCalendarPicker() {
        ...
        // Check if app password is connected first
        const appPasswordInput = document.getElementById('appPasswordInline');
        if (appPasswordInput) {
            // If the input exists, it means they're NOT connected
            console.log('⚠️ [Compose] App password not connected, showing warning');
            
            // Shake the input field
            appPasswordInput.style.animation = 'shake 0.5s';
            setTimeout(() => appPasswordInput.style.animation = '', 500);
            
            // Add red border and flash effect
            appPasswordInput.style.border = '2px solid #dc3545';
            appPasswordInput.style.boxShadow = '0 0 10px rgba(220, 53, 69, 0.5)';
            
            // Change placeholder text
            const originalPlaceholder = appPasswordInput.placeholder;
            appPasswordInput.placeholder = 'ENTER APP PASSWORD';
            appPasswordInput.style.color = '#dc3545';
            
            // Reset after animation
            setTimeout(() => {
                appPasswordInput.style.border = '';
                appPasswordInput.style.boxShadow = '';
                appPasswordInput.placeholder = originalPlaceholder;
                appPasswordInput.style.color = '';
            }, 3000);
            
            // Focus the input to draw attention
            appPasswordInput.focus();
            
            // Also scroll to the app password section if needed
            const appPasswordRow = appPasswordInput.closest('.app-password-row');
            if (appPasswordRow) {
                appPasswordRow.scrollIntoView({ behavior: 'smooth', block: 'center' });
            }

    ...
    function renderQuestDetail(quest) {
        ...
        if (conditions.length > 0) {
            conditionsHtml = conditions.map((cond, index) => {
                ...
                // Format can be "reply_contains:text" or just "any_reply"
                let conditionType = condObj.condition || 'any_reply';
                let conditionValue = condObj.value || '';
                
                // Check if condition includes a colon (e.g., "reply_contains:text")
                if (conditionType.includes(':')) {
                    const parts = conditionType.split(':');
                    conditionType = parts[0];
                    conditionValue = parts.slice(1).join(':'); // Rejoin in case value contains colons
                }
